...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Int64(optionNameStorageDelFileTime, 60, "storage service, time between clearing unpinned files, in minutes")
	cmd.Flags().Int(optionNameStorageBlockSize, 8<<20, "storage service, block size when downloading files")
	cmd.Flags().Int(optionNameStorageRetryNumber, 20, "storage service, number of retries when downloading a file fails")
