...
const (
	...
	optionNameStorageEnable             = "storage-enable"
	optionNameStorageSkipOracleRegister = "storage-skip-oracle-register"
	optionNameStorageCapacity           = "storage-capacity"
	optionNameStorageCacheDir           = "storage-cache-dir"
	optionNameStorageDelFileTime        = "storage-del-file-time"
	optionNameStorageBlockSize          = "storage-block-size"
	optionNameStorageRetryNumber        = "storage-retry-number"
	optionNameStorageRedundantDisk      = "storage-redundant-disk"
	optionNameStorageGroupName          = "storage-group-name"
)
...
func (c *command) setAllFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Bool(optionNameStorageEnable, false, "enable storage files")
	cmd.Flags().Bool(optionNameStorageSkipOracleRegister, false, "storage service skip oracle register")
	cmd.Flags().Uint64(optionNameStorageCapacity, 100<<30, "storage service, total storage space available")
	cmd.Flags().String(optionNameStorageCacheDir, "", "storage service, the running tasks, staging directory")
	cmd.Flags().Int64(optionNameStorageDelFileTime, 0, "storage service, time between clearing unpinned files, in minutes, the default not running")
	cmd.Flags().Int(optionNameStorageBlockSize, 8<<20, "storage service, block size when downloading files")
	cmd.Flags().Int(optionNameStorageRetryNumber, 5, "storage service, number of retries when downloading a file fails")
	cmd.Flags().Uint64(optionNameStorageRedundantDisk, 300<<20, "storage service, stop the service when available disk space is less than redundant space")
	cmd.Flags().String(optionNameStorageGroupName, "", "group name of the storage service")
}
