...
func testDBCollectGarbageWorker(t *testing.T) {
	...
	t.Run("get the file reference chunk", func(t *testing.T) {
		_, err := db.Get(context.Background(), storage.ModeGetRequest, reference, 0)
	...
	t.Run("file related chunks should be removed", func(t *testing.T) {
		for i := 0; i < chunkCount; i++ {
			_, err := db.Get(context.Background(), storage.ModeGetRequest, chunks[i], 0)
			if !errors.Is(err, storage.ErrNotFound) {
				t.Errorf("got error %v, want %v", err, storage.ErrNotFound)
			}
		}
...
func TestPinGC(t *testing.T) {
	...
	t.Run("first upload chunk was removed from gc index", func(t *testing.T) {
		for _, hash := range chunksA {
			_, err := db.Get(context.Background(), storage.ModeGetRequest, hash, 0)
			if !errors.Is(err, storage.ErrNotFound) {
				t.Fatal(err)
			}
		}
...
func TestGCAfterPin(t *testing.T) {
	...
	for _, hash := range chunks {
		_, err := db.Get(ctx, storage.ModeGetRequest, hash, 0)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestDB_collectGarbageWorker_withRequests(t *testing.T) {
	...
	_, err := db.Get(ctx, storage.ModeGetRequest, addrs[0], 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("get requested chunk", func(t *testing.T) {
		_, err := db.Get(context.Background(), storage.ModeGetRequest, addrs[0], 0)
		if err != nil {
			t.Fatal(err)
		}
	})
	...
	t.Run("get gc-ed chunk", func(t *testing.T) {
		_, err := db.Get(context.Background(), storage.ModeGetRequest, addrs[1], 0)
		if !errors.Is(err, storage.ErrNotFound) {
			t.Errorf("got error %v, want %v", err, storage.ErrNotFound)
		}
	})
	...
	t.Run("get most recent synced chunk", func(t *testing.T) {
		_, err := db.Get(context.Background(), storage.ModeGetRequest, addrs[len(addrs)-1], 0)
		if err != nil {
			t.Fatal(err)
		}
	})
...
func TestPinAfterMultiGC(t *testing.T) {
	...
	for _, addr := range pinnedChunks {
		...
		gotChunk, err := db.Get(ctx, storage.ModeGetRequest, boson.NewAddress(outItem.Address), 0)
		if err != nil {
			t.Fatal(err)
		}
...
func TestPinSyncAndAccessPutSetChunkMultipleTimes(t *testing.T) {
	...
	for _, ch := range pinnedChunks {
		gotChunk, err := db.Get(context.Background(), storage.ModeGetRequest, ch.Address(), 0)
		if err != nil {
...
func addRandomChunks(t *testing.T, count int, db *DB, pin bool) []boson.Chunk {
	...
	for i := 0; i < count; i++ {
		...
		if pin {
			...
			_, err = db.Get(ctx, storage.ModeGetRequest, ch.Address(), 0)
			if err != nil {
				t.Fatal(err)
			}
		} else {
			...
			_, _ = db.Get(ctx, storage.ModeGetRequest, ch.Address(), 0)
		}
...
func TestGC_NoEvictDirty(t *testing.T) {
	...
	go func() {
		...
		for range incomingChan {
			...
			if i < 2 {
				...
				_, err := db.Get(ctx, storage.ModeGetRequest, addrs[i], 0)
				mtx.Unlock()
				if err != nil {
					t.Error(err)
				}
	...
	t.Run("get the first two chunks, third is gone", func(t *testing.T) {
		_, err := db.Get(sctx.SetRootHash(context.Background(), addrs[0]), storage.ModeGetRequest, addrs[0], 0)
		if err != nil {
			t.Error("got error but expected none")
		}
		_, err = db.Get(sctx.SetRootHash(context.Background(), addrs[1]), storage.ModeGetRequest, addrs[1], 0)
		if err != nil {
			t.Error("got error but expected none")
		}
		_, err = db.Get(sctx.SetRootHash(context.Background(), addrs[2]), storage.ModeGetRequest, addrs[2], 0)
		if !errors.Is(err, storage.ErrNotFound) {
	...
	t.Run("only later inserted chunks should be removed", func(t *testing.T) {
		for i := 2; i < (chunkCount - int(gcTarget)); i++ {
			_, err := db.Get(sctx.SetRootHash(context.Background(), addrs[i]), storage.ModeGetRequest, addrs[i], 0)
			if !errors.Is(err, storage.ErrNotFound) {
				t.Errorf("got error %v, want %v", err, storage.ErrNotFound)
			}
		}
	...
	// last synced chunk should not be removed
	t.Run("get most recent synced chunk", func(t *testing.T) {
		_, err := db.Get(sctx.SetRootHash(context.Background(), addrs[len(addrs)-1]), storage.ModeGetRequest, addrs[len(addrs)-1], 0)
		if err != nil {
			t.Fatal(err)
		}
	})
...
func TestPinAndUnpinChunk(t *testing.T) {
	...
	for _, v := range chunks1 {
		_, err := db.Get(rctx, storage.ModeGetRequest, v, 0)
		if err == nil {
...
func TestBrokenPyramid(t *testing.T) {
	...
	for _, chunk := range chunks {
		_, err := db.Get(rctx, storage.ModeGetRequest, chunk, 0)
		if err == nil {
