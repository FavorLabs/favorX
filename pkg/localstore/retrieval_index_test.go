...
//
// # go test -benchmem -run=none github.com/ethersphere/boson/storage/localstore -bench BenchmarkRetrievalIndexes -v
// goos: darwin
// goarch: amd64
// pkg: github.com/ethersphere/boson/storage/localstore
...
func benchmarkRetrievalIndexes(b *testing.B, o *Options, count int) {
	b.StopTimer()
	db := newTestDB(b, o)
	addrs := make([]boson.Address, count)
	for i := 0; i < count; i++ {
	...
	for i := 0; i < count; i++ {
		...
		_, err = db.Get(context.Background(), storage.ModeGetRequest, addrs[i], 0)
		if err != nil {
			b.Fatal(err)
		}
...
//
// go test -benchmem -run=none github.com/ethersphere/boson/storage/localstore -bench BenchmarkUpload -v
// goos: darwin
// goarch: amd64
// pkg: github.com/ethersphere/boson/storage/localstore
