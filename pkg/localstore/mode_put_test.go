...
func TestModePutUpload_parallel(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			chunksChan := make(chan []boson.Chunk)
...
func TestPutDuplicateChunks(t *testing.T) {
	for _, mode := range []storage.ModePut{
		...
		t.Run(mode.String(), func(t *testing.T) {
			...
			got, err := db.Get(context.Background(), storage.ModeGetLookup, ch.Address(), 0)
			if err != nil {
				t.Fatal(err)
			}
...
//
// # go test -benchmem -run=none github.com/ethersphere/boson/storage/localstore -bench BenchmarkPutUpload -v
//
// goos: darwin
// goarch: amd64
