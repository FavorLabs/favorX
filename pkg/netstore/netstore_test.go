...
func TestNetstoreRetrieval(t *testing.T) {
	...
	_, err := nstore.Get(context.Background(), storage.ModeGetRequest, addr, 0)
	if err != nil {
		t.Fatal(err)
	}
	if !retrieve.called {
	...
	d, err := store.Get(context.Background(), storage.ModeGetRequest, addr, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	d, err = nstore.Get(context.Background(), storage.ModeGetRequest, addr, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNetstoreNoRetrieval(t *testing.T) {
	...
	c, err := nstore.Get(context.Background(), storage.ModeGetRequest, addr, 0)
	if err != nil {
		t.Fatal(err)
	}
	if retrieve.called {
...
func TestRecovery(t *testing.T) {
	...
	_, err := nstore.Get(ctx, storage.ModeGetRequest, addr, 0)
...
func (r *retrievalMock) RetrieveChunk(ctx context.Context, rootAddr, addr boson.Address, index int64) (chunk boson.Chunk, err error) {
	if r.failure {
