...
func (f Uint64Vector) Get(i uint64) (val uint64, err error) {
	b, err := f.db.Get(fieldKeyPrefixLength, f.indexKey(i))
	if err != nil {
...
func (f Uint64Vector) Put(i, val uint64) (err error) {
	return f.db.Put(fieldKeyPrefixLength, f.indexKey(i), encodeUint64(val))
}

// PutInBatch stores a uint64 value at index i in a batch
// that can be saved later in the database.
func (f Uint64Vector) PutInBatch(batch driver.Batching, i, val uint64) error {
...
// This operation is not goroutine safe.
func (f Uint64Vector) IncInBatch(batch driver.Batching, i uint64) (val uint64, err error) {
	val, err = f.Get(i)
	if err != nil {
		if errors.Is(err, driver.ErrNotFound) {
			val = 0
	...
	val++
	return val, f.PutInBatch(batch, i, val)
}
...
// This operation is not goroutine safe.
// The field is protected from overflow to a negative value.
func (f Uint64Vector) DecInBatch(batch driver.Batching, i uint64) (val uint64, err error) {
	val, err = f.Get(i)
	if err != nil {
		if errors.Is(err, driver.ErrNotFound) {
			val = 0
