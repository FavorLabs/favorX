...
// Ensure multi-error implements Error interface.
...
// New will return a new multi-error.
...
// Append will append errors to the multi-error.
func (e *Error) Append(errs ...error) {
	e.Errors = append(e.Errors, errs...)
}

// ErrorOrNil returns an error interface if the multi-error represents a list of
...
// WrapErrorOrNil will wrap the given error if the multi-error contains errors.
...
// Unwrap is called on the multi-error.Error.
...
func format(errs []error) string {
	...
	for i, err := range errs {
		msg[i] = fmt.Sprintf("%s", err)
	}

	return fmt.Sprintf("%d errors occurred: %s",
		len(errs), strings.Join(msg, ", "))
