...
func TestTopologyAnnounce(t *testing.T) {
	var (
		...
		n1a = func(context.Context, boson.Address, bool) error {
			mtx.Lock()
			announceCalled = true
			mtx.Unlock()
			return nil
		}
		n1at = func(context.Context, boson.Address, boson.Address, bool) error {
			mtx.Lock()
...
func expectZeroAddress(t *testing.T, addrs ...boson.Address) {
	t.Helper()
	for i, a := range addrs {
		if !a.Equal(boson.ZeroAddress) {
			t.Fatalf("address did not equal zero address. index %d", i)
		}
	}
}

func waitAddrSet(t *testing.T, addr *boson.Address, mtx *sync.Mutex, exp boson.Address) {
	t.Helper()
	for i := 0; i < 20; i++ {
		mtx.Lock()
		if addr.Equal(exp) {
...
func checkAddressbook(t *testing.T, ab addressbook.Getter, overlay boson.Address, underlay ma.Multiaddr) {
	t.Helper()
...
func (n *notifiee) Announce(ctx context.Context, a boson.Address, full bool) error {
	return n.announce(ctx, a, full)
}

func (n *notifiee) AnnounceTo(ctx context.Context, a, b boson.Address, full bool) error {
