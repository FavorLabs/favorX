...
func TestPingSuccess(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		reachableFunc func(addr boson.Address, got p2p.ReachabilityStatus)
	}{
		{
			name: "ping success",
			pingFunc: func(context.Context, ma.Multiaddr) (time.Duration, error) {
...
func TestDisconnected(t *testing.T) {
	...
	*/
	...
	reachableFunc := func(addr boson.Address, b p2p.ReachabilityStatus) {}
...
func newMock(ping func(context.Context, ma.Multiaddr) (time.Duration, error), reach func(boson.Address, p2p.ReachabilityStatus)) *mock {
	return &mock{
		pingFunc:      ping,
		reachableFunc: reach,
	}
...
func (m *mock) Reachable(addr boson.Address, status p2p.ReachabilityStatus) {
