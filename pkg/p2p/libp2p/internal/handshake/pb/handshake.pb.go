...
func (m *Ack) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.NodeMode) > 0 {
		i -= len(m.NodeMode)
		copy(dAtA[i:], m.NodeMode)
		i = encodeVarintHandshake(dAtA, i, uint64(len(m.NodeMode)))
		i--
		dAtA[i] = 0x1a
	}
	if m.NetworkID != 0 {
...
func (m *Ack) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field NodeMode", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.NodeMode = append(m.NodeMode[:0], dAtA[iNdEx:postIndex]...)
			if m.NodeMode == nil {
				m.NodeMode = []byte{}
			}
			iNdEx = postIndex
