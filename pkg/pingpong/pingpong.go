...
import (
	...
	"github.com/FavorLabs/favorX/pkg/pingpong/pb"
...
func (s *Service) Ping(ctx context.Context, address boson.Address, msgs ...string) (rtt time.Duration, err error) {
	...
	for _, msg := range msgs {
		...
		logger.Tracef("got pong: %q from %s", pong.Response, address)
...
func (s *Service) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream) error {
	...
	for {
		...
		logger.Tracef("got ping: %q from %s", ping.Greeting, p.Address)
