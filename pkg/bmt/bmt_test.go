...
import (
	...
	"github.com/FavorLabs/favorX/pkg/bmt"
	"github.com/FavorLabs/favorX/pkg/bmt/reference"
...
var (
	...
	hashSize          = boson.NewHasher().Size()
	seed              = time.Now().Unix()
)

func refHash(count int, data []byte) ([]byte, error) {
	rbmt := reference.NewRefHasher(boson.NewHasher(), count)
...
func TestHasherEmptyData(t *testing.T) {
	for _, count := range testSegmentCounts {
		t.Run(fmt.Sprintf("%d_segments", count), func(t *testing.T) {
			...
			pool := bmt.NewPool(bmt.NewConf(boson.NewHasher, count, 1))
			h := pool.Get()
...
func TestSyncHasherCorrectness(t *testing.T) {
	...
	for _, count := range testSegmentCounts {
		t.Run(fmt.Sprintf("segments_%v", count), func(t *testing.T) {
			...
			var incr int
			capacity := 1
			pool := bmt.NewPool(bmt.NewConf(boson.NewHasher, count, capacity))
...
func testHasherReuse(t *testing.T, poolsize int) {
	pool := bmt.NewPool(bmt.NewConf(boson.NewHasher, testSegmentCount, poolsize))
	h := pool.Get()
	defer pool.Put(h)
...
func TestBMTConcurrentUse(t *testing.T) {
	testData := randomBytes(t, seed)
	pool := bmt.NewPool(bmt.NewConf(boson.NewHasher, testSegmentCount, testPoolSize))
	cycles := 100
...
func TestBMTWriterBuffers(t *testing.T) {
	for i, count := range testSegmentCounts {
		t.Run(fmt.Sprintf("%d_segments", count), func(t *testing.T) {
			pool := bmt.NewPool(bmt.NewConf(boson.NewHasher, count, testPoolSize))
			h := pool.Get()
			defer pool.Put(h)
